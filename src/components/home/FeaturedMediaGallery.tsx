...
import { Play, Eye, User, Camera, Video, Clock, ArrowRight, ImageOff, VideoOff } from 'lucide-react';
import Link from 'next/link';

// ─── Photo Empty State ─────────────────────────────────────────────────────────
const PhotoEmptyState = () => (
  <div className="col-span-full flex flex-col items-center justify-center py-20 bg-card/50 rounded-[2rem] border border-border border-dashed text-center gap-4">
    <div className="w-20 h-20 rounded-full bg-accent flex items-center justify-center">
      <ImageOff className="w-9 h-9 text-muted-foreground/40" />
    </div>
    <div>
      <p className="text-lg font-bold text-foreground">No Photography Available</p>
      <p className="text-sm text-muted-foreground mt-1">Featured photos will appear here once uploaded via the Admin Panel.</p>
    </div>
    <Link
      href="/admin/media/photos/create"
      className="mt-2 px-5 py-2 bg-brand-orange/10 hover:bg-brand-orange/20 text-brand-orange text-sm font-bold rounded-full transition-colors border border-brand-orange/20"
    >
      Upload First Photo →
    </Link>
  </div>
...
const VideoEmptyState = () => (
  <div className="col-span-full flex flex-col items-center justify-center py-20 bg-card/50 rounded-[2rem] border border-border border-dashed text-center gap-4">
    <div className="w-20 h-20 rounded-full bg-accent flex items-center justify-center">
      <VideoOff className="w-9 h-9 text-muted-foreground/40" />
    </div>
    <div>
      <p className="text-lg font-bold text-foreground">No Videos Available Yet</p>
      <p className="text-sm text-muted-foreground mt-1">Championship highlights and show videos — coming soon.</p>
    </div>
    <span className="mt-2 px-5 py-2 bg-accent text-muted-foreground text-sm font-bold rounded-full border border-border">
      Coming Soon
    </span>
  </div>
);
...
const SectionHeader = ({
  ...
  <div className="flex justify-between items-end border-b border-border pb-5">
    <div>
      <span className="text-brand-orange font-bold text-xs uppercase tracking-widest flex items-center gap-2 mb-2">
    ...
    <Link
      href={viewAllHref}
      className="text-brand-orange hover:text-orange-600 font-bold transition-colors flex items-center gap-1.5 group text-sm md:text-base shrink-0 ml-4"
    >
...
export default function FeaturedMediaGallery({ photos, videos }: FeaturedMediaGalleryProps) {
  return (
    <section className="w-full overflow-hidden pb-8 md:pb-12 lg:pb-16 pt-0 bg-background text-foreground font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 space-y-20">
                ...
                <Link href={`/gallery/show-photos/details?slug=${photo.slug}`}>
                  <div className="group bg-card rounded-[1.75rem] border border-border overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col cursor-pointer">
                    ...
                    <div className="p-5 flex flex-col gap-3">
                      <h3 className="font-bold text-foreground group-hover:text-brand-orange transition-colors line-clamp-1 text-base leading-snug">
                        {photo.title}
                      </h3>
                      ...
                      <div className="flex items-center justify-between border-t border-border pt-3 text-xs font-semibold text-muted-foreground">
                        ...
                        <span className="text-brand-orange text-[10px] font-bold uppercase tracking-wide group-hover:underline">
                          View Photo
                        </span>
                      </div>
                    </div>
              ...
              >
                <Link href={`/gallery/show-videos/details?slug=${video.slug}`}>
                  <div className="group bg-card rounded-[1.75rem] border border-border overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col cursor-pointer">
                    ...
                    {/* Info */}
                    <div className="p-5 flex flex-col gap-3">
                      <h3 className="font-bold text-foreground group-hover:text-brand-orange transition-colors line-clamp-2 text-base leading-snug">
                        {video.title}
                      </h3>
