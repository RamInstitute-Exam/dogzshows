...
import { Camera, Menu, X, Search, Bell, Download, MapPin, Phone, Mail, ChevronDown, Settings, LogOut } from 'lucide-react';
...
export default function HomeHeader() {
  ...
  return (
    <header className="sticky top-0 z-[1000] flex flex-col w-full glass-panel">
      {/* Main Navigation */}
      <nav className="w-full h-[64px] md:h-[72px] lg:h-[80px] flex items-center justify-between px-4 sm:px-6 lg:px-8 max-w-[1600px] mx-auto">
        {/* Logo */}
        ...
        {/* Desktop Navigation */}
        <div className="hidden lg:flex items-center gap-[32px] px-4 flex-1 justify-center">
          {navLinks.map((link) => {
            ...
            return (
              <Link 
                ...
                href={link.href} 
                className={cn(
                  "font-bold text-[15px] transition-all whitespace-nowrap py-2 relative",
                  isActive 
                    ? "text-[#F59E0B] after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-[#F59E0B]" 
                    : "text-[#CBD5E1] hover:text-foreground"
                )}
              >
                {link.name}
              </Link>
        ...
        <div className="hidden lg:flex items-center space-x-4 shrink-0">
          ...
          {!isAuthenticated ? (
            <div className="flex items-center gap-3">
              ...
              <Button className="bg-card hover:bg-accent border border-border text-foreground rounded-xl px-6 h-[48px] font-bold shadow-md text-sm">
                Register Dog
              </Button>
            </div>
          ) : (
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button 
                  ...
                  className="flex items-center gap-2 pl-1 pr-3 py-1 rounded-full border border-border hover:border-border bg-input transition-all h-10"
                >
                ...
                <AnimatePresence>
                  {isAvatarDropdownOpen && (
                    ...
                    >
                      <div className="p-4 border-b border-border bg-[#0B1220]">
                      ...
                      <div className="p-2">
                        {renderView().map(link => (
                          <Link key={link.name} href={link.href} className="flex items-center px-4 py-2 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-brand-orange rounded-xl transition-colors">
                            {link.name}
                          </Link>
                        ))}
                      </div>
                      <div className="p-2 border-t border-border">
                        <button className="w-full flex items-center px-4 py-2 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-foreground rounded-xl transition-colors">
                          <Settings className="w-4 h-4 mr-2" /> Settings
                        </button>
                        <button onClick={logout} className="w-full flex items-center px-4 py-2 text-sm font-medium text-red-500 hover:bg-red-500/10 rounded-xl transition-colors">
        ...
        <div className="lg:hidden flex items-center gap-4 shrink-0">
          {!isAuthenticated && (
            <Button size="sm" onClick={() => openModal('LOGIN')} className="btn-primary-luxury h-12 rounded-xl px-5 text-sm">
              Login
            </Button>
          )}
          <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 rounded-full text-foreground hover:bg-card/5">
          ...
          <div className="fixed inset-0 z-[9999] lg:hidden flex justify-start">
            ...
            >
              <div className="p-6 flex justify-between items-center border-b border-border">
                ...
                />
                <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-card/5 rounded-full text-foreground hover:bg-card/10">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="p-6 border-t border-border space-y-4 bg-[#0B1220]">
                {!isAuthenticated ? (
                  ...
                  <>
                    <div className="flex items-center gap-3 mb-4 p-4 bg-input rounded-xl shadow-sm border border-border">
                    ...
                    <Button className="w-full min-h-[48px] rounded-xl bg-card border border-border hover:bg-accent text-foreground font-bold text-base">Go to Dashboard</Button>
                    <Button onClick={() => {setIsMobileMenuOpen(false); logout();}} variant="outline" className="w-full min-h-[48px] rounded-xl font-bold text-red-500 border-red-500/20 bg-red-500/10 hover:bg-red-500/20">Sign Out</Button>
                  </>
                )}
              </div>
            </motion.div>
