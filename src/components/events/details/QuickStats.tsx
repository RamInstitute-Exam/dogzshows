...
export default function QuickStats({ event }: { event: any }) {
  const stats = [
    ...
    { label: 'Registration Ends', value: event.closingDate, icon: Clock },
    { label: 'Expected Dogs', value: event.expectedDogs, icon: Activity },
    { label: 'Total Judges', value: event.judgeCount, icon: Users },
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 py-12">
      {stats.map((stat, i) => (
        ...
        >
          <div className="w-14 h-14 bg-orange-50 text-brand-orange rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
