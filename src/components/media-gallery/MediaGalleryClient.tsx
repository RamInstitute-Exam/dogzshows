          ...
          {!defaultCategorySlug && (
            <div className="flex bg-accent p-1.5 rounded-full w-fit border border-border self-start">
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold transition-all ${activeTab === 'photos' ? 'bg-[#F59E0B] text-foreground shadow-md' : 'text-muted-foreground hover:text-foreground'}`}
              >
                <Camera className="w-4 h-4" /> Photos
              </button>
              <button
                onClick={() => { setActiveTab('videos'); setSelectedCategory('all'); setSelectedAlbum('all'); }}
                className={`flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold transition-all ${activeTab === 'videos' ? 'bg-[#F59E0B] text-foreground shadow-md' : 'text-muted-foreground hover:text-foreground'}`}
              >
          ...
          <div className="flex flex-wrap items-center gap-4 w-full xl:w-auto">
            ...
            {!defaultCategorySlug && (
              <div className="flex flex-col min-w-[160px] w-full sm:w-auto">
                <select
                  ...
                  className="px-4 py-3 bg-background border border-border rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-[#F59E0B]/50 transition-all cursor-pointer"
                >
            ...
            <div className="relative flex-grow sm:flex-grow-0 sm:w-72">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-background border border-border rounded-full text-sm font-semibold outline-none focus:ring-2 focus:ring-[#F59E0B]/50 transition-all"
              />
            </div>
          </div>
        </div>
              ...
              {activeTab === 'photos' ? (
                ...
                images.map((photo: any, idx: number) => (
                  <motion.div
                    ...
                    className="bg-card rounded-[2rem] border border-border overflow-hidden cursor-pointer group hover:shadow-2xl transition-all duration-300 flex flex-col justify-between"
                  ...
                  >
                    <div className="relative aspect-[4/3] w-full overflow-hidden bg-accent">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <div className="w-12 h-12 rounded-full bg-[#F59E0B] text-foreground flex items-center justify-center transform scale-75 group-hover:scale-100 transition-all duration-300 shadow-lg">
                          <ZoomIn className="w-5 h-5 text-foreground" />
                        </div>
                      </div>
                      <span className="absolute top-4 left-4 bg-black/60 backdrop-blur-md text-foreground text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                        {photo.category?.name}
                      </span>
                    </div>
                    ...
                    <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
                      <div>
                        <h3 className="text-lg font-bold font-outfit text-foreground leading-snug line-clamp-1 group-hover:text-[#F59E0B] transition-colors">
                          {photo.title}
                        </h3>
                      ...
                      <div className="flex items-center justify-between border-t border-border pt-4 text-xs font-bold text-muted-foreground">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-full hover:bg-[#F59E0B]/10 hover:text-[#F59E0B] transition-colors active:scale-95"
                        >
                  ...
                  >
                    <div className="relative aspect-video w-full overflow-hidden bg-accent">
                      ...
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center transition-colors group-hover:bg-black/50">
                        <div className="w-14 h-14 rounded-full bg-[#F59E0B] text-foreground flex items-center justify-center shadow-lg group-hover:scale-110 transform transition-transform duration-300">
                          <Play className="w-6 h-6 fill-current text-foreground ml-1" />
                        </div>
                      </div>
                      <span className="absolute bottom-3 right-3 bg-black/85 backdrop-blur-md text-foreground text-[10px] font-bold px-2 py-1 rounded-md">
            ...
            >
              ...
              <div className="p-6 md:p-8 md:w-1/3 lg:w-1/4 flex flex-col justify-between border-t md:border-t-0 md:border-l border-border bg-card overflow-y-auto min-h-0 flex-grow md:flex-grow-0">
                <div className="space-y-6">
                  <div>
                    <span className="bg-[#F59E0B]/20 text-[#F59E0B] px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-3 inline-block">
                  ...
                  <div className="flex flex-wrap gap-4 text-sm font-semibold text-muted-foreground">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-accent hover:bg-[#F59E0B]/10 hover:text-[#F59E0B] transition-colors"
                    >
                ...
                <div className="flex gap-4 pt-6 border-t border-border mt-8">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#F59E0B] hover:bg-orange-600 font-bold rounded-xl transition-colors text-sm text-foreground"
                  >
            ...
            >
              ...
              <div className="p-6 md:p-8 bg-card flex flex-col justify-between">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <span className="bg-[#F59E0B]/20 text-[#F59E0B] px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-2 inline-block">
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-accent hover:bg-[#F59E0B]/10 hover:text-[#F59E0B] rounded-xl transition-colors text-sm font-semibold"
                    >
