...
export default function CommitteeMembersPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get('/club-committees');
      if (res.success) {
        setData(res.data);
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete('/club-committees/' + id);
      fetchData();
    } catch (err) {}
  };
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/club-committees', {
      ...
      <main className="flex-1 md:ml-64  bg-background text-muted-foreground p-8">
        <div className="w-full space-y-4">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-3xl font-bold text-foreground mb-2">Committee Members</h1>
              <p className="text-muted-foreground">Manage committee members</p>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-card border-b border-border text-muted-foreground uppercase text-xs tracking-wider">
                    ...
                    <th className="p-4 font-semibold capitalize">designation</th>
                    <th className="p-4 font-semibold capitalize">email</th>
                    <th className="p-4 font-semibold capitalize">phone</th>
                ...
                <tbody className="divide-y divide-[rgba(255,255,255,0.05)]">
                  {data.map((item) => (
                    <tr key={item.id} className="hover:bg-accent transition-colors">
                      ...
                      <td className="p-4 text-foreground font-bold">{item.designation}</td>
                      <td className="p-4 text-foreground font-bold">{item.email}</td>
                      <td className="p-4 text-foreground font-bold">{item.phone}</td>
