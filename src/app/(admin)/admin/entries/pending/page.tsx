...
import { Search, RefreshCw, CheckCircle, XCircle, Eye, Edit, X, Loader2, FileText, User, CalendarDays } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-6 rounded-2xl border border-border shadow-xl">
        <div>
          <h1 className="text-3xl font-extrabold text-foreground tracking-tight flex items-center gap-3">
            <RefreshCw className="w-8 h-8 text-yellow-500 animate-spin-slow" /> Pending Entries
          </h1>
          <p className="text-muted-foreground font-medium mt-1">Review newly submitted show entries awaiting validation. Total: {totalCount}</p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-card border-b border-border">
                ...
                <th className="py-4 px-6 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right font-black">Audit / Actions</th>
              </tr>
            </thead>
                ...
                entries.map((entry, idx) => (
                  ...
                  >
                    <td className="py-4 px-6">
                      <p className="font-mono text-xs text-yellow-500 font-bold">#{entry.id.substring(0, 8).toUpperCase()}</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-card max-w-2xl w-full rounded-2xl border border-border shadow-2xl overflow-hidden flex flex-col text-foreground">
              <div className="flex justify-between items-center p-6 border-b border-border shrink-0 bg-accent/10">
                <h3 className="text-xl font-extrabold text-foreground">Audit Pending Entry</h3>
              ...
              <div className="p-6 border-t border-border bg-accent/10 flex justify-end gap-2 shrink-0">
                <Button onClick={() => handleApprove(selectedEntry.id)} className="bg-green-600 hover:bg-green-700 text-white font-bold px-5">Approve Submission</Button>
                <Button onClick={() => handleReject(selectedEntry.id)} className="bg-red-600 hover:bg-red-700 text-white font-bold px-5">Reject Submission</Button>
