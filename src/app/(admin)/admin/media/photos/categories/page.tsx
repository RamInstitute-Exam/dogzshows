...
export default function PhotoCategoriesManagement() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (formData.id) {
        const res = await api.put(`/photo-categories/${formData.id}`, formData);
        if (res.success) {
        ...
        const res = await api.post('/photo-categories', formData);
        if (res.success) {
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      const res = await api.delete(`/photo-categories/${item.id}`);
      if (res.success) {
  ...
  return (
    <div className=" space-y-4 relative">
      <AdminDataTable
        ...
        description="Configure media gallery categories for organizing photos."
        icon={FolderOpen}
        data={categories}
        columns={columns}
        loading={isLoading}
