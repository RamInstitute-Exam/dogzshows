  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pb-6 border-b border-gray-200/60 gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Adoption & Marketplace Directory</h1>
        ...
        <div className="flex items-center space-x-3 flex-wrap gap-2">
          ...
          <div className="flex items-center bg-gray-100 p-1.5 rounded-xl border border-gray-200/40">
            <button
              onClick={() => setViewMode('grid')}
              className={`p-2 rounded-lg cursor-pointer transition-all ${
                viewMode === 'grid' ? 'bg-white text-indigo-600 shadow-xs' : 'text-gray-500 hover:text-gray-900'
              }`}
              title="Grid View"
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg cursor-pointer transition-all ${
                viewMode === 'list' ? 'bg-white text-indigo-600 shadow-xs' : 'text-gray-500 hover:text-gray-900'
              }`}
              title="List View"
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg cursor-pointer transition-all ${
                viewMode === 'map' ? 'bg-white text-indigo-600 shadow-xs' : 'text-gray-500 hover:text-gray-900'
              }`}
              title="Map View"
            >
          ...
          <div className="relative inline-flex items-center">
            <select
              ...
              className="appearance-none bg-white border border-gray-200 text-gray-700 text-xs font-semibold rounded-xl pl-3 pr-8 py-2.5 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
            >
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="w-full lg:w-68 flex-shrink-0 bg-white p-5 rounded-2xl border border-gray-200/60 shadow-xs h-fit sticky lg:top-24 max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-5">
            {/* Search Input */}
            <div>
              <label className="block text-2xs font-bold text-gray-400 uppercase tracking-wider mb-2">Search Query</label>
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-xs rounded-xl pl-9 pr-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                />
              </div>
            </div>
            ...
            {/* Breed Dropdown */}
            <div>
              <label className="block text-2xs font-bold text-gray-400 uppercase tracking-wider mb-2">Breed Category</label>
              <select
                value={selectedBreed}
                onChange={(e) => setSelectedBreed(e.target.value)}
                className="w-full bg-gray-50 border border-gray-200 text-gray-700 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium"
              >
            ...
            {/* Gender Switch */}
            <div>
              <label className="block text-2xs font-bold text-gray-400 uppercase tracking-wider mb-2">Gender</label>
              <div className="grid grid-cols-3 gap-1.5 p-1 bg-gray-150/70 bg-gray-100 rounded-xl border border-gray-200/20 text-center text-xs font-semibold">
                {['All', 'MALE', 'FEMALE'].map((g) => (
                  <button
                    ...
                    className={`py-1.5 rounded-lg cursor-pointer transition-all ${
                      selectedGender === g ? 'bg-white text-indigo-700 shadow-xs' : 'text-gray-500'
                    }`}
                  >
            ...
            {/* Listing Type Switch */}
            <div>
              <label className="block text-2xs font-bold text-gray-400 uppercase tracking-wider mb-2">Listing Type</label>
              <div className="grid grid-cols-3 gap-1.5 p-1 bg-gray-100 rounded-xl border border-gray-200/20 text-center text-xs font-semibold">
                {['All', 'ADOPTION', 'SALE'].map((t) => (
                  <button
                    ...
                    className={`py-1.5 rounded-lg cursor-pointer transition-all ${
                      selectedListingType === t ? 'bg-white text-indigo-700 shadow-xs' : 'text-gray-500'
                    }`}
                  >
            ...
            {/* Price Inputs */}
            <div>
              <label className="block text-2xs font-bold text-gray-400 uppercase tracking-wider mb-2">Price Limit (INR)</label>
              <div className="flex items-center space-x-2">
                <input
                  ...
                  className="w-1/2 bg-gray-50 border border-gray-200 text-gray-900 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium"
                />
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-2xs font-bold text-gray-400 uppercase tracking-wider">Maximum Age</label>
        ...
        <div className="flex-1 space-y-6">
          ...
          {aiRecommendation && !searchQuery && selectedBreed === 'All' && (
            <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-purple-950 p-6 rounded-3xl border border-indigo-400/20 shadow-xl relative overflow-hidden text-white flex flex-col md:flex-row items-center gap-6">
              ...
              <div>
                ...
                <p className="text-gray-300 text-xs leading-relaxed mt-2.5 max-w-xl line-clamp-2">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="bg-white rounded-2xl border border-gray-150 p-4 space-y-4 animate-pulse">
                  ...
                  <div className="pt-3 border-t border-gray-50 flex justify-between items-center">
                    <div className="h-3.5 bg-gray-100 rounded-md w-1/4"></div>
                    <div className="h-5 bg-gray-100 rounded-full w-1/5"></div>
                  </div>
                </div>
              ))}
            </div>
          ) : sortedDogs.length === 0 ? (
            <div className="text-center py-24 bg-white border border-gray-200/65 rounded-3xl">
              <Compass className="mx-auto h-12 w-12 text-gray-300 mb-3" />
              <h3 className="text-lg font-bold text-gray-800">No companions matching query</h3>
              <p className="text-gray-500 text-sm mt-1">Try relaxing filters, adjusting price settings, or matching broad breeds.</p>
            </div>
          ) : (
            <>
              {/* GRID MODE */}
                  ...
                  {sortedDogs.map((dog) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="aspect-w-4 aspect-h-3 bg-gray-50 relative overflow-hidden h-48">
                          ...
                          {/* Verification Badges */}
                          {dog.registered && (
                            <span className="absolute top-3 left-3 bg-indigo-600/90 text-white text-3xs font-extrabold px-2 py-0.5 rounded-full flex items-center shadow-xs">
                          ...
                          <button
                            ...
                            className="absolute top-3 right-3 p-2 bg-white/90 backdrop-blur rounded-full text-gray-400 hover:text-rose-500 shadow-xs cursor-pointer hover:scale-105 transition-all"
                          >
                          ...
                          <button
                            ...
                            className={`absolute bottom-3 right-3 px-2 py-1 bg-white/95 backdrop-blur text-3xs font-bold rounded-lg shadow-xs cursor-pointer border hover:border-indigo-400 transition-all ${
                              isComparing ? 'border-indigo-500 text-indigo-700 bg-indigo-50' : 'border-gray-200 text-gray-500'
                            }`}
                          >
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                          <div className="flex justify-between items-start mb-1">
                            <h3 className="font-extrabold text-base text-gray-900 group-hover:text-indigo-600 transition-colors pr-2 truncate">
                              {dog.name}
                            </h3>
                          ...
                          <div className="mt-4 pt-3.5 border-t border-gray-100 flex items-center justify-between text-xs font-semibold text-gray-750">
                ...
                <div className="space-y-4">
                  {sortedDogs.map((dog) => {
                    ...
                    return (
                      ...
                      >
                        <div className="h-44 w-full sm:w-56 bg-gray-50 relative flex-shrink-0">
                          {dog.images && dog.images.length > 0 ? (
                          ...
                          <button
                            type="button"
                            onClick={(e) => handleToggleFavorite(dog.id, e)}
                            className="absolute top-3 right-3 p-2 bg-white/90 backdrop-blur rounded-full text-gray-400 hover:text-rose-500 shadow-xs cursor-pointer transition-colors"
                          >
                          ...
                          <div>
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <span className="inline-block bg-indigo-50 text-indigo-700 text-3xs font-extrabold uppercase tracking-wide border border-indigo-100 rounded-md px-1.5 py-0.5 mt-1">
                                  {dog.breed}
                                </span>
                              </div>
                              <span className="font-extrabold text-sm text-gray-900 bg-gray-50 border border-gray-200/50 px-2.5 py-1 rounded-xl">
                                {dog.listingType === 'SALE' ? `INR ${dog.price.toLocaleString()}` : 'Adoption'}
                              </span>
                            </div>

                            <p className="text-gray-500 text-xs mt-3 leading-relaxed max-w-xl line-clamp-2">
                              {dog.description}
                            </p>
                          </div>

                          <div className="mt-6 flex items-center space-x-4 text-xs font-semibold text-gray-500">
              ...
              {viewMode === 'map' && (
                <div className="bg-white rounded-3xl border border-gray-200/60 shadow-sm overflow-hidden flex flex-col h-[70vh]">
                  <div className="flex-1 bg-[url('https://images.unsplash.com/photo-1524661135-423995f22d0b?auto=format&fit=crop&q=80&w=1200')] bg-cover bg-center relative flex items-center justify-center p-4">
                    ...
                    {sortedDogs.slice(0, 4).map((dog, i) => {
                      ...
                      return (
                        <div
                          key={dog.id}
                          className="absolute bg-white/95 backdrop-blur shadow-2xl p-2 rounded-2xl flex items-center space-x-2 border border-indigo-200 animate-bounce duration-1000"
                        ...
                        >
                          ...
                          <div>
                            <p className="text-3xs font-extrabold text-gray-900">{dog.name}</p>
                            <p className="text-4xs text-indigo-600 font-bold">{dog.breed}</p>
                          </div>
                        </div>
                      );
                    })}

                    <div className="absolute bottom-6 right-6 bg-white/90 backdrop-blur-md px-4 py-3 rounded-2xl shadow-xl max-w-xs border border-gray-100">
                      <h4 className="font-bold text-xs text-gray-900">Map Directory Overlay</h4>
                      <p className="text-4xs text-gray-500 font-medium mt-0.5">Showing mock geo-location mappings for active listing addresses.</p>
                    </div>
                  </div>
                </div>
              )}
          ...
          {recentlyViewed.length > 0 && (
            <div className="pt-6 border-t border-gray-200/60">
              <h3 className="font-extrabold text-sm text-gray-900 mb-4 flex items-center space-x-1.5">
                ...
                {recentlyViewed.map((dog) => (
                  ...
                  >
                    ...
                    <div className="overflow-hidden">
                      <p className="text-2xs font-bold text-gray-900 truncate group-hover:text-indigo-600 transition-colors">{dog.name}</p>
                      <p className="text-3xs text-gray-400 truncate mt-0.5">{dog.breed}</p>
                    </div>
                  </Link>
                ))}
              </div>
      ...
      {isCompareOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/40 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-3xl shadow-2xl border border-gray-100 flex flex-col space-y-6 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center pb-3 border-b border-gray-100">
              ...
              <button
                ...
                className="text-gray-400 hover:text-gray-650 hover:bg-gray-100 p-1.5 rounded-lg cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100 text-center border border-gray-100 rounded-xl overflow-hidden">
                <thead className="bg-gray-50/50 text-xs font-bold text-gray-500">
                  <tr>
                ...
                <tbody className="bg-white divide-y divide-gray-100 text-xs font-semibold text-gray-700">
                  <tr>
                    <td className="px-4 py-3 text-left font-bold text-gray-500">Breed</td>
                    {compareList.map((dog) => (
                      <td key={dog.id} className="px-4 py-3">{dog.breed}</td>
                    ))}
                  </tr>
                  <tr>
                    <td className="px-4 py-3 text-left font-bold text-gray-500">Age</td>
                    {compareList.map((dog) => (
                      <td key={dog.id} className="px-4 py-3">{dog.age} years</td>
                    ))}
                  </tr>
                  <tr>
                    <td className="px-4 py-3 text-left font-bold text-gray-500">Gender</td>
                    {compareList.map((dog) => (
                      <td key={dog.id} className="px-4 py-3">{dog.gender === 'MALE' ? 'Male' : 'Female'}</td>
                    ))}
                  </tr>
                  <tr>
                    <td className="px-4 py-3 text-left font-bold text-gray-500">Listing</td>
                    {compareList.map((dog) => (
                      <td key={dog.id} className="px-4 py-3">{dog.listingType === 'SALE' ? `INR ${dog.price.toLocaleString()}` : 'Adoption'}</td>
                    ))}
                  </tr>
                  <tr>
                    <td className="px-4 py-3 text-left font-bold text-gray-500">Vaccinated</td>
                    {compareList.map((dog) => (
                      <td key={dog.id} className="px-4 py-3">{dog.vaccinated ? 'Yes ✓' : 'No ✗'}</td>
                    ))}
                  </tr>
                  <tr>
                    <td className="px-4 py-3 text-left font-bold text-gray-500">Pedigree</td>
                    {compareList.map((dog) => (
            ...
            <div className="pt-4 flex justify-end space-x-3">
              <button
                ...
                className="bg-gray-50 hover:bg-gray-100 text-gray-650 font-semibold px-5 py-2.5 rounded-xl transition-all border border-gray-200 text-xs cursor-pointer"
              >
                Clear Compare
              </button>
              <button
                onClick={() => setIsCompareOpen(false)}
                className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-5 py-2.5 rounded-xl transition-all text-xs cursor-pointer"
              >
                Done
              </button>
            </div>
