...
export default function InboxPage() {
  ...
  return (
    <ProtectedRoute>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full h-[calc(100vh-8rem)] flex flex-col">
        ...
        <div className="mb-6">
          ...
          <p className="text-xs text-gray-500 font-medium mt-0.5">Communicate directly with registered breeders and adoption leads.</p>
        </div>

        <div className="flex-1 bg-white border border-gray-200/60 rounded-3xl overflow-hidden shadow-sm flex flex-col md:flex-row">
          
          {/* LEFT THREAD LIST PANEL */}
          <aside className="w-full md:w-80 border-b md:border-b-0 md:border-r border-gray-150 flex flex-col bg-gray-50/20">
            <div className="p-4 border-b border-gray-100 bg-gray-50/50">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Conversations ({threads.length})</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
              {loadingThreads ? (
                ...
                <div className="text-center py-12 px-4">
                  ...
                  <p className="text-xs font-bold text-gray-400">Your inbox is empty</p>
                  <p className="text-3xs text-gray-400 mt-1 leading-relaxed">Submit interest on dog listings to start chatting.</p>
                </div>
              ) : (
                threads.map((thread) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-4.5 p-4 flex items-center space-x-3 transition-colors cursor-pointer ${
                        isActive ? 'bg-indigo-50/40 text-indigo-700' : 'hover:bg-gray-50'
                      }`}
                    >
                      ...
                      <div className="overflow-hidden flex-1">
                        <div className="flex justify-between items-baseline">
                          <p className={`text-xs font-semibold truncate ${isActive ? 'text-indigo-900' : 'text-gray-900'}`}>
                            {thread.partner.email}
                          </p>
                        </div>
                        <p className={`text-3xs truncate mt-1 leading-normal ${isUnread ? 'font-bold text-gray-900' : 'text-gray-500'}`}>
          ...
          <main className="flex-1 flex flex-col justify-between bg-white relative">
            {selectedPartnerId ? (
              <>
                {/* Chat Header */}
                <div className="p-4 border-b border-gray-150 bg-gray-55 bg-gray-50/30 flex items-center justify-between">
                  <div className="flex items-center space-x-2.5">
                    ...
                    <div>
                      <p className="text-xs font-bold text-gray-900">{activeThread?.partner.email}</p>
                      <span className="text-4xs text-emerald-600 font-semibold flex items-center">
                ...
                {/* Messages log */}
                <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-gray-50/10">
                  {loadingMessages ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <span
                              className={`block text-4xs mt-1.5 text-right font-medium ${
                ...
                {/* Message input */}
                <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-150 bg-white flex items-center space-x-3.5">
                  <input
                    type="text"
                    required
                    ...
                    className="w-full bg-gray-50 border border-gray-200 focus:bg-white text-gray-950 text-xs rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-semibold"
                  />
                  <button
                    type="submit"
                    disabled={sendingMessage || !newMessage.trim()}
                    className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white p-3 rounded-xl transition-all shadow-md flex justify-center items-center cursor-pointer flex-shrink-0"
                  >
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-50/10">
                ...
                <h3 className="font-extrabold text-gray-800 text-base">Select a conversation</h3>
                <p className="text-xs text-gray-500 max-w-xs mt-1">Select an active contact thread on the sidebar panel to view chat session history logs.</p>
              </div>
            )}
          </main>
