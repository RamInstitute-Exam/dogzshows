...
export default function EditEventClient() {
  ...
  const handleAddCustomJudge = () => {
    setCustomJudges([...customJudges, { judge_name: '' }]);
  };

  const handleCustomJudgeChange = (index: number, value: string) => {
    const updated = [...customJudges];
    updated[index].judge_name = value;
    setCustomJudges(updated);
  };

  const handleRemoveCustomJudge = (index: number) => {
    const updated = [...customJudges];
  ...
  const fetchData = async () => {
    ...
    try {
      const [clubsRes, judgesRes, eventRes, fciGroupsRes] = await Promise.all([
        api.get('/public/clubs?limit=1000'),
        api.get('/public/judges?limit=1000'),
        api.get(`/shows/${id}`),
        api.get('/public/fci-groups?limit=100')
      ]);
      ...
      let allFciGroups: any[] = [];
      if (fciGroupsRes?.success) {
        allFciGroups = fciGroupsRes.data || [];
        setFciGroups(allFciGroups);
      }
      ...
      if (eventRes?.success && eventRes.data) {
        ...
        // Map Group Limits
        if (allFciGroups.length > 0) {
          const limitsMap = new Map();
          (ev.groupLimits || []).forEach((gl: any) => {
            limitsMap.set(gl.fciGroupId, gl.maxLimit);
          });
          setGroupLimits(allFciGroups.map((g: any) => ({
            fciGroupId: g.id,
            name: g.name,
            maxLimit: limitsMap.has(g.id) ? limitsMap.get(g.id) : 30
          })));
        }
      } else {
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {activeTab === 'settings' && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                    ...
                    {/* CUSTOM EVENT JUDGES SECTION */}
                    <div className="space-y-4 pt-6 border-t border-border">
                      <div className="flex justify-between items-center pb-3">
                        <div>
                          <h3 className="text-xl font-bold text-foreground">Custom Event Judges</h3>
                          <p className="text-xs text-muted-foreground mt-0.5">Some events have guest judges who are not available in the Master Judges database.</p>
                        </div>
                        <AdminButton
                          type="button"
                          variant="outline"
                          size="sm"
                          onClick={handleAddCustomJudge}
                      ...
                      {customJudges.length > 0 && (
                        <div className="space-y-4">
                          {customJudges.map((cj, idx) => (
                            <div key={idx} className="flex items-center gap-3 bg-card border border-border p-3 rounded-xl">
                              <div className="flex-1">
                                <label className="block text-[11px] font-bold text-muted-foreground mb-1">Judge Name *</label>
                                <input
                                  type="text"
                                  placeholder="e.g. Mr. Guest Judge"
                                  value={cj.judge_name}
                                  onChange={(e) => handleCustomJudgeChange(idx, e.target.value)}
                                  required
                              ...
                              <button
                                type="button"
                                onClick={() => handleRemoveCustomJudge(idx)}
                                className="mt-4 p-2 text-muted-foreground hover:text-red-500 hover:bg-red-50 rounded"
                              >
                                <Trash2 className="w-4 h-4" />
                              </button>
                            </div>
