...
export default function SpecialAwardList() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/${'special-awards'}?page=${page}&limit=20&search=${search}`);
      if (res.success) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await api.delete(`/${'special-awards'}/${id}`);
      if (res.success) {
    ...
    <div className="w-full max-w-[1400px] mx-auto px-4 py-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-6 rounded-2xl border border-border shadow-sm">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage special awards and recognition</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <Link href={`/admin/winners/special-awards/create`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-muted/50 text-muted-foreground uppercase font-bold text-xs">
              <tr>
                <th className="px-6 py-4">Award Name</th>
              ...
              {loading ? (
                ...
                data.map(item => (
                  <tr key={item.id} className="hover:bg-muted/50 transition-colors">
                    <td className="px-6 py-4 font-medium text-foreground">{item.awardName}</td>
                    <td className="px-6 py-4 font-medium text-foreground">{item.description}</td>
                    <td className="px-6 py-4 font-medium text-foreground">{item.image}</td>
                    ...
                    <td className="px-6 py-4 flex items-center justify-end gap-2">
                      <Link href={`/admin/winners/special-awards/edit?id=${item.id}`}>
