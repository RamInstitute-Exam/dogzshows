...
export default function EditSpecialAward() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(`/${'special-awards'}/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await api.put(`/special-awards/${id}`, formData);
      if (res.success) {
        toast.success('Updated successfully!');
        router.push('/admin/winners/special-awards');
      } else {
    ...
    <div className="w-full max-w-[1000px] mx-auto px-4 py-6 space-y-6">
      <div className="flex justify-between items-center bg-card p-6 rounded-2xl border border-border shadow-sm sticky top-4 z-50">
        <div className="flex items-center gap-4">
          <Link href="/admin/winners/special-awards">
            <button className="text-muted-foreground hover:text-foreground hover:bg-accent rounded-xl p-2 transition">
              <ArrowLeft className="w-5 h-5" />
            </button>
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-foreground tracking-tight">Edit Special Awards</h1>
          </div>
        </div>
      ...
      <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-bold text-muted-foreground mb-2">Award Name *</label>
              <input type="text" name="awardName" value={formData.awardName} onChange={handleInputChange} required className="w-full px-4 py-3 bg-background border border-border rounded-lg focus:border-foreground outline-none" />
            </div>
            ...
            <div>
              <label className="block text-sm font-bold text-muted-foreground mb-2">Image</label>
              {formData.image ? (
                <div className="flex flex-col gap-2">
                  <img src={formData.image} className="h-32 object-contain rounded border border-border bg-muted/50" />
                  <button type="button" onClick={() => setFormData(prev => ({...prev, image: ''}))} className="text-red-500 text-sm font-bold text-left">Remove Image</button>
                </div>
              ) : (
