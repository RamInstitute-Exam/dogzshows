...
const getWinnerEventId = (w: any) =>
  w.event?.id || w.eventId || slugify(w.eventName || w.event?.name || 'general');

export default function EventGalleryClient({ eventId }: EventGalleryClientProps) {
  ...
  useEffect(() => {
    async function loadWinners() {
      try {
        ...
        const filtered = all.filter(w => getWinnerEventId(w) === eventId);

        if (filtered.length > 0) {
          const first = filtered[0];
          setEventName(first.event?.name || first.eventName || first.awardTitle || 'Championship Show');
          setClubName(first.club?.name || first.event?.club?.name || first.clubName || '');
        }
